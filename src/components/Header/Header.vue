<template>
  <div v-divHeight>
    <div class="Top transparent">
      ...
      <form
        action="http://www.jius.net/Search/Search.asp"
        name="searchform"
        method="post"
      ...
      <div class="text" v-scrollClick="closeLogoutFlag">
        <span v-show="testLoginStateFlag" v-tap="{methods:showLogin}" id="LoginTxt">
          <p>{{loginState}}</p>
        </span>
        <mt-spinner v-show="!testLoginStateFlag" type="fading-circle"></mt-spinner>
        <span v-show="logoutFlag" v-tap="{methods:logout}" id="logoutTxt">
          <p>退出登录</p>
        </span>
      </div>
    </div>
